...
class MultiNodeSync:
    ...
    def sync_to_node(self, node):
        """
        Synchronize files to a specific node
        ...
        try:
            ...
            # Use cp for local network transfer
            sync_cmd = [
                'cp', 
                '-R',  # Recursive copy
                '-p',  # Preserve attributes
                '--remove-destination',  # Remove existing destination files
                f'{self.config["shared_directory"]}/.', 
                remote_path
            ]
...
def main():
    ...
    parser.add_argument(
        '--config', 
        default='/etc/multi_node_sync.json', 
